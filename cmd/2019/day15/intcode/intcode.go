...
var ErrSigTerm = errors.New("intcode program terminated")
...
func Run(program Program, input <-chan int, output chan<- int) (<-chan ExitStatus, chan<- struct{}) {
	exit, sigterm := make(chan ExitStatus), make(chan struct{})
		...
		for {
			...
			default:
				...
				case 4:
					...
					case <-sigterm:
						return
					case output <- mem.read(instr.params[0]):
						ptr += 2
					}
